...
fn resolve_repo_root(cli_value: Option<PathBuf>) -> anyhow::Result<PathBuf> {
    ...
    // Common case: launched from ebpf-tui/ which is inside 26_EBPF-TUI/.
    // The module folders (01_BPF_PROG_TYPE_* ... 25_BPF_PROG_TYPE_*) are two levels up.
    // ebpf-tui/ -> 26_EBPF-TUI/ -> repo_root/
    if cwd
        .file_name()
        .and_then(|s| s.to_str())
        .map(|s| s == "ebpf-tui")
        .unwrap_or(false)
    {
        // Go up two levels: ebpf-tui -> 26_EBPF-TUI -> repo root
        if let Some(parent) = cwd.parent().and_then(|p| p.parent()) {
            if looks_like_repo_root(parent) {
                return Ok(parent.to_path_buf());
            }
        }
        // Fallback: one level up (if structure is flat)
    ...
    // If user runs from any subfolder, try to locate a parent directory
    ...
    // manifest_dir = .../26_EBPF-TUI/ebpf-tui, so go up two levels
    if let Some(grandparent) = manifest_dir.parent().and_then(|p| p.parent()) {
        if looks_like_repo_root(grandparent) {
